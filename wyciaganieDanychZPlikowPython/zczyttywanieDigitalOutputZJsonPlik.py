...
def zczyttywanieDigitalOutputZJson(filename):
    ...
    for json1 in input_jsons:
        ...
        if FaceplateType == "Faceplates\\Devices\\Icons\\DO_Icon.FPT":
            #print(json1)
            ...
            new_json = {
                ...
                'DIGITAL_OUTPUT_Status': json1['props']['DIGITAL_OUTPUT_Status'],
                'DIGITAL_OUTPUT_Status02': json1['props']['DIGITAL_OUTPUT_Status02'],
                'DIGITAL_OUTPUT_Config': json1['props']['DIGITAL_OUTPUT_Config'],
                'DIGITAL_OUTPUT_StatusAlarm': json1['props']['DIGITAL_OUTPUT_StatusAlarm'],
    ...
    file_path = f'lokalizacja_obiektów_DigitalOutput_{nameOfFileTemp[0]}.txt'
    with open(file_path, 'w') as file:
        file.write(file_content)


    vbs_script_content = ""
    dictionary_array = "\tDim DOArray(" + str(len(new_jsons)) + ")\n"
    ...
    for i, nj in enumerate(new_jsons):
        ...
        vbs_script_content += f"\tDim DOTab_{i}({len(nj)})\n"
        d = 0
        for key, value in nj.items():
            if (isinstance(value, int) or is_number(value)):  # Jeśli wartość jest intem
                vbs_script_content += f"\tDOTab_{i}({d}) = {value.replace(',', '.')} '{key}\n"
            elif (key != "Label") and (key != "Tag"):  # Jeśli wartość jest stringiem i warunki
                vbs_script_content += f"\tDOTab_{i}({d}) = SmartTags(\"{value}\") '{key}\n"
            else:
                vbs_script_content += f"\tDOTab_{i}({d}) = \"{value}\" '{key} \n"
            
            d=d+1


        vbs_script_content += "\n"
        dictionary_array += f"\tDOArray({i}) = DOTab_{i}\n"


    vbs_script_content =  vbs_script_content + "\n" + dictionary_array  + "\n" + '\tinitializeDO_IconWebUXW2A DOArray\n'

    # Zapis do pliku "proporties_of_objects.vbs"
    vbs_file_path = f'proporties_of_objects_DigitalOutput_{nameOfFileTemp[0]}.txt'
    if len(new_jsons) > 0:
        with open(vbs_file_path, 'w') as file:
            file.write(f'\nSub initialize_DigitalOutput_{nameOfFileTemp[0]}_WebUX(lineName, fullLineName)\n')
            file.write(vbs_script_content)
            file.write(f'\nEnd Sub\n')
    
        return vbs_file_path, f'initialize_DigitalOutput_{nameOfFileTemp[0]}_WebUX'
    else:
        return None, None