...
def zczyttywaniePIDZJson(filename):
    ...
    for json1 in input_jsons:
        ...
        if FaceplateType == "Faceplates\\Devices\\Icons\\PID_Icon.FPT":
            #print(json1)
            ...
            new_json = {
                ...
                'PV_Unit': json1['props']['PV_Unit'],
                'SP_Unit': json1['props']['SP_Unit'],
                

                
                'PID_ProcessData_SPRampOut': json1['props']['PID_ProcessData_SPRampOut'],
                'PID_ProcessData_PVLagOut': json1['props']['PID_ProcessData_PVLagOut'],
                'PID_ProcessData_PID_RegulatorCV': json1['props']['PID_ProcessData_PID_RegulatorCV'],
                'PID_Status': json1['props']['PID_Status'],
                'PID_StatusAlarm': json1['props']['PID_StatusAlarm'],
                'PID_Config': json1['props']['PID_Config'],
    ...
    file_path = f'lokalizacja_obiektów_PID_{nameOfFileTemp[0]}.txt'
    with open(file_path, 'w') as file:
        file.write(file_content)


    vbs_script_content = ""
    dictionary_array = "\tDim pidArray(" + str(len(new_jsons)) + ")\n"
    ...
    for i, nj in enumerate(new_jsons):
        ...
        vbs_script_content += f"\tDim pidTab_{i}({len(nj)})\n"
        d = 0
        for key, value in nj.items():
            if (isinstance(value, int) or is_number(value)):  # Jeśli wartość jest intem
                vbs_script_content += f"\tpidTab_{i}({d}) = {value.replace(',', '.')} '{key}\n"
            elif (key != "Label") and (key != "Tag") and (key != "PV_Unit") and (key != "SP_Unit"):  # Jeśli wartość jest stringiem i warunki
                vbs_script_content += f"\tpidTab_{i}({d}) = SmartTags(\"{value}\") '{key}\n"
            else:
                vbs_script_content += f"\tpidTab_{i}({d}) = \"{value}\" '{key} \n"
            
            d=d+1


        vbs_script_content += "\n"
        dictionary_array += f"\tpidArray({i}) = pidTab_{i}\n"


    vbs_script_content =  vbs_script_content + "\n" + dictionary_array  + "\n" + '\tinitializePID_IconWebUXW2A pidArray\n'

    # Zapis do pliku "proporties_of_objects.vbs"
    vbs_file_path = f'proporties_of_objects_PID_{nameOfFileTemp[0]}.txt'
    if len(new_jsons) > 0:
        with open(vbs_file_path, 'w') as file:
            file.write(f'\nSub initialize_PID_{nameOfFileTemp[0]}_WebUX(lineName, fullLineName)\n')
            file.write(vbs_script_content)
            file.write(f'\nEnd Sub\n')
    
        return vbs_file_path, f'initialize_PID_{nameOfFileTemp[0]}_WebUX'
    else:
        return None, None