...
def zczyttywaniearrow_ElectricHeater_A01(filename):
    ...
    for json1 in input_jsons:
        ...
        if FaceplateType == "Faceplates\\Devices\\Icons\\Electric_Heater_A01.FPT":
            #print(json1)
            ...
            new_json = {
                ...
                'FillColor': json1['props']['FillColor'],

                'Value': json1['props']['Value'],
                
                'Name': json1['props']['Name'],
    ...
    file_content = ""
    for i, nj in enumerate(new_jsons):
        file_content += f"Obiekt_{i+1} ({nj['Name']}):\n  Left: {nj['Left']}\n  Top: {nj['Top']}\n\n"

    nameOfFileTemp = filename.split('.')
    file_path = f'lokalizacja_obiektów_ElectricHeater_A01_{nameOfFileTemp[0]}.txt'
    with open(file_path, 'w') as file:
        file.write(file_content)


    vbs_script_content = ""
    dictionary_array = "\tDim electricHeater_A01Array(" + str(len(new_jsons)) + ")\n"
    ...
    for i, nj in enumerate(new_jsons):
        ...
        vbs_script_content += f"\tDim electricHeater_A01Tab_{i}({len(nj)})\n"
        d = 0
        for key, value in nj.items():
            if (isinstance(value, int) or is_number(value)):  # Jeśli wartość jest intem
                vbs_script_content += f"\telectricHeater_A01Tab_{i}({d}) = {value.replace(',', '.')} '{key}\n"
            elif (key != "Name"):  # Jeśli wartość jest stringiem i warunki
                vbs_script_content += f"\telectricHeater_A01Tab_{i}({d}) = SmartTags(\"{value}\") '{key}\n"
            else:
                vbs_script_content += f"\telectricHeater_A01Tab_{i}({d}) = \"{value}\" '{key} \n"
            
            d=d+1


        vbs_script_content += "\n"
        dictionary_array += f"\telectricHeater_A01Array({i}) = electricHeater_A01Tab_{i}\n"


    vbs_script_content =  vbs_script_content + "\n" + dictionary_array  + "\n" + '\tinitializeElectricHeater_A01_IconWebUXW2A electricHeater_A01Array\n'

    # Zapis do pliku "proporties_of_objects.vbs"
    vbs_file_path = f'proporties_of_objects_ElectricHeater_A01_{nameOfFileTemp[0]}.txt'
    if len(new_jsons) > 0:
        with open(vbs_file_path, 'w') as file:
            file.write(f'\nSub initialize_ElectricHeater_A01_{nameOfFileTemp[0]}_WebUX(lineName, fullLineName)\n')
            file.write(vbs_script_content)
            file.write(f'\nEnd Sub\n')
    
        return vbs_file_path, f'initialize_ElectricHeater_A01_{nameOfFileTemp[0]}_WebUX'
    else:
        return None, None