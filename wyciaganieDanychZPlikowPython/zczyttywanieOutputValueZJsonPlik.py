...
def zczyttywanieOutputValueZJson(filename):
    ...
    for json1 in input_jsons:
        if 'OutputValue' not in json1['props']:
            continue
        
        #print(json1['props']['ObjectName'])

        new_json = {
            'ObjectName': json1['props']['ObjectName'],
            'OutputValue': json1['props']['OutputValue'],
    ...
    vbs_script_content = ""
    dictionary_array = "\tDim OutputArray(" + str(len(new_jsons)) + ")\n"
    ...
    for i, nj in enumerate(new_jsons):
        ...
        vbs_script_content += f"\tDim OutputTab_{i}({len(nj)})\n"
        d = 0
        for key, value in nj.items():


            if (isinstance(value, int) or is_number(value)):  # Jeśli wartość jest intem
                vbs_script_content += f"\tOutputTab_{i}({d}) = {value.replace(',', '.')} '{key}\n"
            elif (key != "ObjectName"):  # Jeśli wartość jest stringiem i warunki
                vbs_script_content += f"\tOutputTab_{i}({d}) = SmartTags(\"{value}\") '{key}\n"
            else:
                vbs_script_content += f"\tOutputTab_{i}({d}) = \"{value}\" '{key} \n"
            
            d=d+1


        vbs_script_content += "\n"
        dictionary_array += f"\tOutputArray({i}) = OutputTab_{i}\n"


    vbs_script_content =  vbs_script_content + "\n" + dictionary_array  + "\n" + '\tinitializeOutputWebUXW2A OutputArray\n'

    # Zapis do pliku "proporties_of_objects.vbs"
    vbs_file_path = f'proporties_of_objects_Output_{nameOfFileTemp[0]}.txt'
    if len(new_jsons) > 0:
        with open(vbs_file_path, 'w') as file:
            file.write(f'\nSub initialize_Output_{nameOfFileTemp[0]}_WebUX(lineName, fullLineName)\n')
            file.write(vbs_script_content)
            file.write(f'\nEnd Sub\n')
    
        return vbs_file_path, f'initialize_Output_{nameOfFileTemp[0]}_WebUX'
    else:
        return None, None
