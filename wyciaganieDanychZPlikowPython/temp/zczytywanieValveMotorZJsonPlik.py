...
def zczytywanieValveMotorZJson(filename):
    ...
    for json1 in input_jsons:
        ...
        if FaceplateType == "Faceplates\\Devices\\Icons\\ValveMotor_Icon.FPT":
            print(json1)
    ...
    nameOfFaceplate = "ValveMotor_Icon"
    kindOfDevice = "ValveMotor_Icon"


    nameOfFileTemp = filename.split('.')
    file_path = f'lokalizacja_obiektów_'+ kindOfDevice + f'_{nameOfFileTemp[0]}.txt'
    ...
    vbs_script_content = ""
    dictionary_array = "\tDim "+ kindOfDevice + "Array(" + str(len(new_jsons)) + ")\n"
    ...
    for i, nj in enumerate(new_jsons):
        ...
        vbs_script_content += f"\tDim "+ kindOfDevice + f"Tab_{i}({len(nj)})\n"
        d = 0
        for key, value in nj.items():
            if (isinstance(value, int) or is_number(value)):  # Jeśli wartość jest intem
                vbs_script_content += f"\t"+ kindOfDevice + f"Tab_{i}({d}) = {value.replace(',', '.')} '{key}\n"
            elif (key != "Tag") and (key != "FullTagName")and (key != "Label"):  # Jeśli wartość jest stringiem i warunki
                vbs_script_content += f"\t"+ kindOfDevice + f"Tab_{i}({d}) = SmartTags(\"{value}\") '{key}\n"
            else:
                vbs_script_content += f"\t"+ kindOfDevice + f"Tab_{i}({d}) = \"{value}\" '{key} \n"
            
            d=d+1


        vbs_script_content += "\n"
        dictionary_array += f"\t"+ kindOfDevice + f"Array({i}) = "+ kindOfDevice + f"Tab_{i}\n"


    vbs_script_content =  vbs_script_content + "\n" + dictionary_array  + "\n" + '\tinitialize_'+ nameOfFaceplate + '_WebUX_W2A '+ kindOfDevice + 'Array\n'


    # Zapis do pliku "proporties_of_objects.vbs"
    vbs_file_path = f'proporties_of_objects_'+ kindOfDevice + f'_{nameOfFileTemp[0]}.txt'
    if len(new_jsons) > 0:
        with open(vbs_file_path, 'w') as file:
            file.write(f'\nSub initialize_'+ kindOfDevice + f'_{nameOfFileTemp[0]}_WebUX(lineName, fullLineName)\n')
            file.write(vbs_script_content)
            file.write(f'\nEnd Sub\n')
    
        return vbs_file_path, f'initialize_'+ kindOfDevice + f'_{nameOfFileTemp[0]}_WebUX'
    else:
        return None, None
