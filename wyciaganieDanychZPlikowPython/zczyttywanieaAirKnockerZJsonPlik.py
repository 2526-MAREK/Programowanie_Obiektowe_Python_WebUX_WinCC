...
def zczyttywanieaAirKnockerZJson(filename):
    ...
    for json1 in input_jsons:
        ...
        if FaceplateType == "Faceplates\\Devices\\Icons\\AirKnocker_Icon.FPT":
            #print(json1)
    ...
    file_path = f'lokalizacja_obiektów_AirKnocker_{nameOfFileTemp[0]}.txt'
    with open(file_path, 'w') as file:
        file.write(file_content)


    vbs_script_content = ""
    dictionary_array = "\tDim airKnocker_IconArray(" + str(len(new_jsons)) + ")\n"
    ...
    for i, nj in enumerate(new_jsons):
        ...
        vbs_script_content += f"\tDim airKnocker_IconTab_{i}({len(nj)})\n"
        d = 0
        for key, value in nj.items():
            if (isinstance(value, int) or is_number(value)):  # Jeśli wartość jest intem
                vbs_script_content += f"\tairKnocker_IconTab_{i}({d}) = {value.replace(',', '.')} '{key}\n"
            elif (key != "Tag") and (key != "FullTagName"):  # Jeśli wartość jest stringiem i warunki
                vbs_script_content += f"\tairKnocker_IconTab_{i}({d}) = SmartTags(\"{value}\") '{key}\n"
            else:
                vbs_script_content += f"\tairKnocker_IconTab_{i}({d}) = \"{value}\" '{key} \n"
            
            d=d+1


        vbs_script_content += "\n"
        dictionary_array += f"\tairKnocker_IconArray({i}) = airKnocker_IconTab_{i}\n"


    vbs_script_content =  vbs_script_content + "\n" + dictionary_array  + "\n" + '\tinitializeAirKnocker_IconWebUXW2A airKnocker_IconArray\n'

    # Zapis do pliku "proporties_of_objects.vbs"
    vbs_file_path = f'proporties_of_objects_AirKnocker_{nameOfFileTemp[0]}.txt'
    if len(new_jsons) > 0:
        with open(vbs_file_path, 'w') as file:
            file.write(f'\nSub initialize_AirKnocker_{nameOfFileTemp[0]}_WebUX(lineName, fullLineName)\n')
            file.write(vbs_script_content)
            file.write(f'\nEnd Sub\n')
    
        return vbs_file_path, f'initialize_AirKnocker_{nameOfFileTemp[0]}_WebUX'
    else:
        return None, None